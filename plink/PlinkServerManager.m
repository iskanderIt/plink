...
@implementation PlinkServerManager

//SRWebSocket *_webSocket;
//NSMutableArray *_messages;

NSString* SERVER_URL =  @"http://192.168.145.1:9000";
...
-(void) openServerChannel:(NSString*) token completionHandler:(void (^)(NSURLResponse *response, NSData *data, NSError *error)) handler{
    ...
    NSURL *url = [NSURL URLWithString:@"http://192.168.145.1:9000/auth"];
...
-(void) addConversation:(PlinkConversation*) conversation withUsers:(NSArray*)users complationhandler:(void (^)(NSString* conversationID)) handler;
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/%@/%@",@"http://192.168.145.1:9000",@"conversations",@"open"]];
...
-(void) addMessage:(PlinkMessage*) message toConversationId:(NSString*) conversationID complationhandler:(void (^)(void)) handler
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/%@/%@",@"http://192.168.145.1:9000",@"send",conversationID]];
